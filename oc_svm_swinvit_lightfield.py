...
transform = transforms.Compose([
        ...
        transforms.Resize((IMG_SIZE, IMG_SIZE)),#224
        transforms.ToTensor(),
        # transforms.Lambda(lambda frames: torch.stack([transforms.ToTensor()(frame) for frame in frames])),
...
class VideoDataset(Dataset):
        ...
        if 'val/fight' in filepath:
            ...
            while True:
                ...
                if annotations.iloc[frame_count, 0] == 1:
                    frame = cv2.resize(frame, (IMG_SIZE // 3, IMG_SIZE // 3))
                    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                    batch_frames.append(Image.fromarray(frame))

                    # frame = transform(frame)
                    # frames.append(frame)
                    if len(batch_frames) == 9:  # if 9 frames are collected
                        big_frame = self.make_big_frame(batch_frames)  # make a big frame
                        frames.append(big_frame)  # add the big frame to frames
                        batch_frames = []
    ...
    def make_big_frame(self, batch_frames):
        rows = [np.hstack(batch_frames[i * 3:i * 3 + 3]) for i in range(3)]
...
def extract_vit(X):
    ...
    with torch.no_grad():
        ...
        for X_tensor_batch in X_tensor:
            ...
            # features_array = features_array.transpose(1, 2)
            features_array = torch.mean(features_array, dim=[1,2])
...
# swin_model = Swinv2Model(configuration).from_pretrained("microsoft/swin-large-patch4-window7-224-in22k").to(device)

swin_model = timm.create_model('swinv2_base_window12to24_192to384', pretrained=True).to(device)
