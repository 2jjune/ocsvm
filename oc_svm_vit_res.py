...
def extract_resnet_2d(X):
    ...
    with torch.no_grad():
        for X_tensor_batch in X_tensor:
            # X_tensor = X_tensor.permute(0,2,1,3,4)
            X_tensor_frame = X_tensor_batch.float()
...
vit_model = nn.Sequential(*list(vit_model.children())[:-2])
...
#         output = self.resnet(src)
#         output = output.view(1,output.shape[0], -1)
#         output = self.transformer_encoder(output)
#         output = self.avgpool(output.transpose(1, 2)).squeeze(2)
#         return output
#
# # Usage:
# combined_model = TransformerFeatureExtractor(d_model=2048, nhead=16, num_layers=6).to(device)
...
for inputs, label in train_loader:#1 = nonfight    ([1, 250, 3, 224, 224])
    ...
    features = np.concatenate((res_feature,incep_feature,features),axis=1)
