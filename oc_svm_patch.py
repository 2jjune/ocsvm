...
from models.i3d.extract_i3d_custom_patch import ExtractI3D
...
def extend_frames(frames, target_length):
    while len(frames) < target_length:
        # 리스트를 반전시키고 첫 번째와 마지막 요소를 제외합니다.
        # 이렇게 하면 [1, 2, 3, 4, 5]가 [4, 3, 2, 1]로 변환됩니다.
        reversed_frames = frames[-2:0:-1]

        # 원본 리스트에 반전된 리스트를 연결합니다.
        frames.extend(reversed_frames)

    # 필요한 길이에 도달하면 리스트를 잘라서 목표 길이에 맞춥니다.
...
res_feature = np.array(res_feature).reshape(len(train_loader),-1)
i3d_feature = (train_features['rgb'] + train_features['flow']).reshape(len(train_loader),-1)

all_features = np.concatenate((res_feature,i3d_feature),axis=1)
...
res_feature = np.array(res_feature).reshape(len(val_loader),-1)
